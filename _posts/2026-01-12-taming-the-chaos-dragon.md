...
![Grounded execution loop: Dive Pack → Execute → chaos dragon check (“are we thrashing?”) → Salvage + update memory → Restart clean](/assets/img/taming-chaos-dragon-loop.png)
...
![Grounded execution system: intent → dive pack → execute → chaos dragon check → salvage/update memory → ship](/assets/img/grounded-execution-system.png)

Rewrite view (use the existing system as spec + enforce equivalence/safety as you move fast):

![Rewrite with a safety harness: mandate → existing system as spec → dive pack → rewrite → safety harness → chaos dragon check → ship](/assets/img/rewrite-with-safety-harness.png)
